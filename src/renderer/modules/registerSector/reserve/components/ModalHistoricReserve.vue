<template>
  <div>
    <div id="modalHistoricReserve" class="modal" tabindex="-1" role="dialog">
      <div class="modal-dialog modal-xl">
        <div class="modal-content roundbox boxshadow">
          <!-- modal header -->
          <modal-header @closeModal="closeModal" title="HISTÓRICOS DA RESERVA"/>
          <!-- / modal header -->
...
export default {
  ...
  methods: {
    ...
    getData() {
      http
        .get(`register-sector/reserve/historic/${this.dataModal.reserve_id}`)
        .then(results => {
    ...
    create() {
      ...
      const data = {
        text: this.historic,
        id_register: this.dataModal.reserve_id
      };

      http
        .post("register-sector/reserve/historic", data)
        .then(result => {
    ...
    closeModal() {
      $("#modalHistoricReserve").modal("hide");
    }
  },
  watch: {
    dataModal() {
